...
-- 3. 테이블 EMPLOYEES 의 테이블 구조를 조회하는 SQL 문을 작성하시오.
desc employees;
...
-- 6.
-- 테이블 EMPLOYEES 의 SALARY(급여)가 6000을 초과하는 사원의 
...
-- asc desc 생략 : 오름차순이 기본값


-- 9.
-- 테이블 EMPLOYEES 의 JOB_ID가 ‘FI_ACCOUNT’ 이거나 ‘IT_PROG’ 인 사원의 
-- 모든 컬럼을 조회하는 SQL 문을 작성하시오.
SELECT *
  FROM employees
...
-- 11.
-- 테이블 EMPLOYEES 의 JOB_ID가 ‘FI_ACCOUNT’ 이거나 
--‘IT_PROG’ 아닌 사원의 모든 컬럼을 조회하는 SQL 문을 작성하시오.
-- IN 키워드를 사용하여 SQL 쿼리를 완성하시오.
SELECT *
  FROM employees
...
-- 13.
-- 테이블 EMPLOYEES 의 FIRST_NAME 이 ‘S’로 
-- 시작하는 사원의 모든 컬럼을 조회하는 SQL 문을 작성하시오.
-- % : 빈칸, 1글자이상 대체
-- _ : 1글자 대체
SELECT *
  FROM employees
...
-- 15.
-- 테이블 EMPLOYEES 의 FIRST_NAME 에 ‘s’가 포함되는 사원의 
-- 모든 컬럼을 조회하는 SQL 문을 작성하시오.
SELECT *
  FROM employees
 WHERE first_name like '%s%'
...
-- 18.
-- 테이블 EMPLOYEES 의 COMMISSION_PCT가 NULL이 
-- 아닌 사원의 모든 컬럼을 조회하는 SQL 문을 작성하시오.
SELECT *
  FROM employees
...
-- 20.
-- 테이블 EMPLOYEES 의 사원의 HIRE_DATE가 04년도부터 
-- 05년도인 모든 컬럼을 조회하는 SQL 문을 작성하시오.
SELECT *
  FROM employees
 WHERE hire_date >= to_date('04/01/01','YY/MM/DD')
...
-- 125.67 을 일의 자리에서 반올림 하시오.
SELECT ROUND(125.67, -1) FROM dual;
-- 125.67 을 십의 자리에서 반올림 하시오.
...
-- 주민등록번호 뒷자리 1자리로 제외한 나머지 문자를 *로 마스킹하시오.
...
SELECT '20240904' 문자
      ,TO_DATE('20240904','YYYYMMDD') 날짜1
      ,TO_DATE('2024-09-04','YYYY-MM-DD') 날짜2
      ,TO_DATE('2024/09/04','YYYY/MM/DD') 날짜3
      ,TO_DATE('2024.09.04','YYYY.MM.DD') 날짜3
FROM DUAL;
...
SELECT '1,200,000' 문자
      ,TO_NUMBER('1,200,000','999,999,999') 숫자
FROM DUAL;
...
SELECT '2024/08/05' 개강
      ,TO_CHAR(ADD_MONTHS( '2024/08/05', 6 )+6,'YYYY/MM/DD') 종강
FROM DUAL;
...
SELECT SYSDATE 오늘
      ,TO_CHAR(NEXT_DAY( SYSDATE, 1 ),'YY/MM/DD') "다음 일요일"
      ,TO_CHAR(NEXT_DAY( SYSDATE, 2 ),'YY/MM/DD') "다음 월요일"
      ,TO_CHAR(NEXT_DAY( SYSDATE, 3 ),'YY/MM/DD') "다음 화요일"
      ,TO_CHAR(NEXT_DAY( SYSDATE, 4 ),'YY/MM/DD') "다음 수요일"
      ,TO_CHAR(NEXT_DAY( SYSDATE, 5 ),'YY/MM/DD') "다음 목요일"
      ,TO_CHAR(NEXT_DAY( SYSDATE, 6 ),'YY/MM/DD') "다음 금요일"
      ,TO_CHAR(NEXT_DAY( SYSDATE, 7 ),'YY/MM/DD') "다음 토요일"
FROM DUAL;
...
-- DISTINCT : 중복 없이 조회
-- NVL ( 값, 대체할 값 ) : 해당 값이 NULL 이면 지정된 값으로 변환 하는 함수

-- * ORDER BY 의 정령기준 컬럼은, SELECT 에서 선택한 컬럼만 사용 가능하다.
-- 함수 사용시 AS 로 별칭을 주어 사용하는것이 좋다
